<script lang="ts">
	...
	import BeatPresenter from '$lib/device/metronome/BeatPresenter.svelte';
	...
	function initiateRemainedComponent() {
		...
		selectedComponent = components[randInt];
		remainedComponents = components.filter((component) => !Object.is(component, selectedComponent));
	}
</script>

<div class="relative h-full">
	<div class="h-60px z-1 absolute right-0 flex w-screen flex-row items-center justify-between">
      ...
      <div class="flex h-full flex-col items-start justify-between">
		...
		>
			{#if metronome.isRunning}
				<Icon class="color-indigo-1 h-20px w-20px m-0 p-0" src={Stop} theme="solid" />
			{:else}
				<Icon class="color-indigo-1 h-20px w-20px  m-0 p-0" src={Play} theme="solid" />
			{/if}
		</button>
	</div>
	<div class="relative flex h-full flex-col items-center justify-center">
		<button tabindex={-1} on:click={openBox}>
